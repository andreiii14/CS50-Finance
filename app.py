...
def buy():
    ...
    if request.method == "POST":
        ...
        if total_cost > user_cash:
            return apology("Not enough funds to buy the shares")
        ...
        symbol = lookup(stock_info["symbol"])

        return render_template(
            "bought.html",
            symbol=symbol["name"],
            price=float(symbol["price"]),
            value_of_holding=shares * float(symbol["price"]),
...
def sell():
    ...
    if request.method == "POST":
        ...
        transaction_value = query["price"] * shares
